...
class Select
{
     ...
     * or else an array of strings or  Select\Exception objects.
     *
     * @param  array|string|Expr $cols The columns to select from this table.
    ...
    public function columns($cols = '*', $correlationName = null)
    {
        ...
        if (!array_key_exists($correlationName, $this->_parts[self::FROM])) {
            throw new Select\Exception("No table has been specified for the FROM clause");
        }
    ...
    public function getPart($part)
    {
        ...
        if (!array_key_exists($part, $this->_parts)) {
            throw new Select\Exception("Invalid Select part '$part'");
        }
     ...
     * @return PDO_Statement|Statement
     */
    ...
    protected function _join($type, $name, $cond, $cols, $schema = null)
    {
        if (!in_array($type, self::$_joinTypes) && $type != self::FROM) {
            throw new Select\Exception("Invalid join type '$type'");
        }

        if (count($this->_parts[self::UNION])) {
            throw new Select\Exception("Invalid use of table with " . self::SQL_UNION);
        }
        ...
        if (!empty($correlationName)) {
            if (array_key_exists($correlationName, $this->_parts[self::FROM])) {
                throw new Select\Exception("You cannot define a correlation name '$correlationName' more than once");
            }
    ...
    public function _joinUsing($type, $name, $cond, $cols = '*', $schema = null)
    {
        if (empty($this->_parts[self::FROM])) {
            throw new Select\Exception("You can only perform a joinUsing after specifying a FROM table");
        }
    ...
    {
        if (count($this->_parts[self::UNION])) {
            throw new Select\Exception("Invalid use of where clause with " . self::SQL_UNION);
        }
     ...
     *
     * @param string $method
     * @param array $args OPTIONAL Table\Select query modifier
     * @return Select
     * @throws Select\Exception If an invalid method is called.
     */
        ...
        if (preg_match('/^join([a-zA-Z]*?)Using$/', $method, $matches)) {
            ...
            if ($type) {
                ...
                if (!in_array($type, self::$_joinTypes)) {
                    throw new Select\Exception("Unrecognized method '$method()'");
                }
                if (in_array($type, array(self::CROSS_JOIN, self::NATURAL_JOIN))) {
                    throw new Select\Exception("Cannot perform a joinUsing with method '$method()'");
                }
            } else {
