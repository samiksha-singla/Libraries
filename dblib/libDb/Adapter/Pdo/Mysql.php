...
class Mysql extends PdoAbstract {
   ...
   public function limit($sql, $count, $offset = 0) {
      $count = intval($count);
      if ($count <= 0) {
         throw new \libDb\Adapter\Exception("LIMIT argument count=$count is not valid");
      }

      $offset = intval($offset);
      if ($offset < 0) {
         throw new \libDb\Adapter\Exception("LIMIT argument offset=$offset is not valid");
      }
