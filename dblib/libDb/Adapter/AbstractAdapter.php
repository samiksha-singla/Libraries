...
abstract class AbstractAdapter
{
    ...
    /**
     * Query profiler object, of type \libDb\Profiler
     ...
     * @var string
     */
    protected $_defaultStmtClass = '\\libDb\Statement';
     ...
     * @var string
     */
    protected $_defaultProfilerClass = '\\libDb\Profiler';
     ...
     * If true, most SQL generated by \libDb\Db classes applies
            ...
            switch ($case) {
                ...
                default:
                    throw new Exception('Case must be one of the following constants: '
                        . 'libDb\Db::CASE_NATURAL,libDb\Db::CASE_LOWER, libDb\Db::CASE_UPPER');
            }
        }
     ...
     * \libDb\Profiler.
     *
     * An instance of \libDb\Profiler sets the adapter's instance to that
     ...
     * class to use for a custom profiler. The class must be \libDb\Profiler or a
     ...
     * @param  \libDb\Profiler|array|boolean $profiler
     * @return AbstractAdapter Provides a fluent interface
     * @throws \libDb\Profiler\Exception if the object instance or class specified
     *         is not \libDb\Profiler or an extension of that class.
     */
    public function setProfiler($profiler)
    {
        ...
        if ($profilerIsObject = is_object($profiler)) {
            if ($profiler instanceof \libDb\Profiler) {
                $profilerInstance = $profiler;
            }  else {
                throw new \libDb\Profiler\Exception ('Profiler argument must be an instance of either Profiler' );
            }
        }
        ...
        if (!$profilerInstance instanceof \libDb\Profiler) {
            throw new \libDb\Profiler\Exception('Class ' . get_class($profilerInstance) . ' does not extend '
                . 'Profiler');
        }
     ...
     * @return \libDb\Statement\StatementInterface
     */
    public function query($sql, $bind = array())
    {
        ...
        // is the $sql a \\libDb\Select object?
        ...
        // because $bind may be a libDb\Expr object
        if (!is_array($bind)) {
            $bind = array($bind);
        }
    ...
    {
        $this->_connect();
        $q = $this->_profiler->queryStart('begin', \libDb\Profiler::TRANSACTION);
    ...
    {
        $this->_connect();
        $q = $this->_profiler->queryStart('commit', \libDb\Profiler::TRANSACTION);
        $this->_commit();
        $this->_profiler->queryEnd($q);
        return $this;
    }
    ...
    {
        $this->_connect();
        $q = $this->_profiler->queryStart('rollback', \libDb\Profiler::TRANSACTION);
        $this->_rollBack();
        $this->_profiler->queryEnd($q);
        return $this;
    }
    ...
    public function update($table, array $bind, $where = '')
    {
         ...
         * except for \libDb\Expr which is treated literally.
         */
        $set = array();
        $i = 0;
        foreach ($bind as $col => $val) {
    ...
    /**
     * Convert an array, string, or \libDb\Expr object
    ...
    public function quote($value, $type = null)
    {
        ...
        if ($value instanceof \libDb\Expr) {
     ...
     * the adapter.
     *
     * @param string|array|\libDb\Expr $ident The identifier.
     ...
     *
     * @param string|array|\libDb\Expr $ident The identifier or expression.
    ...
    {
        if ($ident instanceof \libDb\Expr) {
            ...
            if (is_array($ident)) {
                ...
                foreach ($ident as $segment) {
                    if ($segment instanceof \libDb\Expr) {
     ...
     * @return \libDb\Statement|\PDOStatement
     */
