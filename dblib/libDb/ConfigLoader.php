...
class ConfigLoader implements \Countable, \Iterator
{
    /**
    ...
    /**
     * ConfigLoader provides a property based interface to
     * an array. The data are read-only unless $allowModifications
     * is set to true on construction.
     *
     * ConfigLoader also implements Countable and Iterator to
    ...
    /**
     * Deep clone of this instance to ensure that nested ConfigLoaders
     * are also cloned.
     *
     * @return void
     */
    ...
    /**
     * Merge another ConfigLoader with this one. The items
     * in $merge will override the same named items in
     * the current config.
     *
     * @param ConfigLoader $merge
     * @return ConfigLoader
     */
    public function merge(ConfigLoader $merge)
    {
        foreach($merge as $key => $item) {
            if(array_key_exists($key, $this->_data)) {
                if($item instanceof ConfigLoader && $this->$key instanceof ConfigLoader) {
                    $this->$key = $this->$key->merge(new ConfigLoader($item->toArray(), !$this->readOnly()));
                } else {
                    $this->$key = $item;
                }
            } else {
                if($item instanceof ConfigLoader) {
                    $this->$key = new ConfigLoader($item->toArray(), !$this->readOnly());
                } else {
                    $this->$key = $item;
                }
            }
     ...
     * after merge() has been used to merge multiple ConfigLoader objects
    ...
    /**
     * Returns if this ConfigLoader object is read only or not.
     *
     * @return boolean
     */
