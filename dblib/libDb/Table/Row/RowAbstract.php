...
abstract class RowAbstract implements \ArrayAccess, \IteratorAggregate {
   ...
   /**
    * \libDb\Table\AbstractTable parent class or instance.
    *
   ...
   /**
    * Name of the class of the \libDb\Table\AbstractTable object.
    *
    * @var string
    */
    ...
    * - table       = class name or object of type \libDb\Table\AbstractTable
   ...
   public function __construct(array $config = array()) {
      if (isset($config['table']) && $config['table'] instanceof \libDb\Table\AbstractTable) {
      ...
      if (isset($config['data'])) {
         if (!is_array($config['data'])) {
            throw new \libDb\Table\Row\Exception('Data must be an array');
         }
    ...
    * @throws \libDb\Table\Row\Exception if the $columnName is not a string.
    */
   protected function _transformColumn($columnName) {
      if (!is_string($columnName)) {
         throw new \libDb\Table\Row\Exception('Specified column is not a string');
      }
    ...
    * @throws \libDb\Table\Row\Exception if the $columnName is not a column in the row.
    */
   public function __get($columnName) {
      $columnName = $this->_transformColumn($columnName);
      if (!array_key_exists($columnName, $this->_data)) {
         throw new \libDb\Table\Row\Exception("Specified column \"$columnName\" is not in the row");
      }
   ...
   public function __unset($columnName) {
      ...
      if ($this->isConnected() && in_array($columnName, $this->_table->info('primary'))) {
         throw new \libDb\Table\Row\Exception("Specified column \"$columnName\" is a primary key and should not be unset");
      }
    ...
    *
    * @return \libDb\Table\AbstractTable|null
    */
   ...
   /**
    * Returns an instance of the parent table's libDb\Table\Select object.
    *
    * @return libDb\Table\Select
    */
   ...
   protected function _doInsert() {
      ...
      if ($this->_readOnly === true) {
         throw new \libDb\Table\Row\Exception('This row has been marked read-only');
      }
      ...
      if (is_array($primaryKey)) {
         ...
         //ZF-6167 Use tempPrimaryKey temporary to avoid that  encoding fails.
   ...
   public function delete() {
      ...
      $this->_data = array_combine(
              array_keys($this->_data), array_fill(0, count($this->_data), null)
      );
    ...
    *
    * @param  array $data
    * @return libDb\Table\Rowset\RowsetAbstract Provides a fluent interface
    */
   ...
   protected function _getTable() {
      if (!$this->_connected) {
         throw new \libDb\Table\Row\Exception('Cannot save a Row unless it is connected');
      }
      return $this->_table;
   }
   ...
   protected function _getPrimaryKey($useDirty = true) {
      if (!is_array($this->_primary)) {
         throw new \libDb\Table\Row\Exception("The primary key must be set as an array");
      }
      ...
      if (count($primary) != count($array)) {
         throw new \libDb\Table\Row\Exception("The specified Table '$this->_tableClass' does not have the same primary key as the Row");
      }
      return $array;
   }
   ...
   protected function _getWhereQuery($useDirty = true) {
      ...
      $metadata = $info[\libDb\Table\AbstractTable::METADATA];

      // retrieve recently updated row using primary keys
      $where = array();
      foreach ($primaryKey as $column => $value) {
         $tableName = $db->quoteIdentifier($info[\libDb\Table\AbstractTable::NAME], true);
   ...
   protected function _refresh() {
      ...
      if (null === $row) {
         throw new \libDb\Table\Row\Exception('Cannot refresh row as parent is missing');
      }
    ...
    *
    * @param \libDb\Table\AbstractTable $dependentTable
    * @param \libDb\Table\AbstractTable $parentTable
    * @param string                 $ruleKey
    * @return array
    */
   protected function _prepareReference(\libDb\Table\AbstractTable $dependentTable, \libDb\Table\AbstractTable $parentTable, $ruleKey) {
      $parentTableName = (get_class($parentTable) === '\libDb\Table') ? $parentTable->getDefinitionConfigName() : get_class($parentTable);
      $map = $dependentTable->getReference($parentTableName, $ruleKey);

      if (!isset($map[\libDb\Table\AbstractTable::REF_COLUMNS])) {
         $parentInfo = $parentTable->info();
         $map[\libDb\Table\AbstractTable::REF_COLUMNS] = array_values((array) $parentInfo['primary']);
      }

      $map[\libDb\Table\AbstractTable::COLUMNS] = (array) $map[\libDb\Table\AbstractTable::COLUMNS];
      $map[\libDb\Table\AbstractTable::REF_COLUMNS] = (array) $map[\libDb\Table\AbstractTable::REF_COLUMNS];
    ...
    *
    * @param string|\libDb\Table\AbstractTable  $dependentTable
    * @param string                         OPTIONAL $ruleKey
    * @param libDb\Table\Select           OPTIONAL $select
    * @return libDb\Table\Rowset\RowsetAbstract Query result from $dependentTable
    * @throws \libDb\Table\Row\Exception If $dependentTable is not a table or is not loadable.
    */
   public function findDependentRowset($dependentTable, $ruleKey = null, libDb\Table\Select $select = null) {
      $db = $this->_getTable()->getAdapter();
      ...
      if (!$dependentTable instanceof \libDb\Table\AbstractTable) {
         $type = gettype($dependentTable);
         if ($type == 'object') {
            $type = get_class($dependentTable);
         }
         throw new \libDb\Table\Row\Exception("Dependent table must be a \libDb\Table\AbstractTable, but it is $type");
      }
      ...
      if (($tableDefinition = $this->_table->getDefinition()) !== null && ($dependentTable->getDefinition() == null)) {
         $dependentTable->setOptions(array(\libDb\Table\AbstractTable::DEFINITION => $tableDefinition));
      }
      ...
      for ($i = 0; $i < count($map[\libDb\Table\AbstractTable::COLUMNS]); ++$i) {
         $parentColumnName = $db->foldCase($map[\libDb\Table\AbstractTable::REF_COLUMNS][$i]);
         ...
         $dependentColumnName = $dependentDb->foldCase($map[\libDb\Table\AbstractTable::COLUMNS][$i]);
         $dependentColumn = $dependentDb->quoteIdentifier($dependentColumnName, true);
         $dependentInfo = $dependentTable->info();
         $type = $dependentInfo[\libDb\Table\AbstractTable::METADATA][$dependentColumnName]['DATA_TYPE'];
    ...
    *
    * @param string|\libDb\Table\AbstractTable $parentTable
    * @param string                        OPTIONAL $ruleKey
    * @param libDb\Table\Select          OPTIONAL $select
    * @return RowAbstract   Query result from $parentTable
    * @throws \libDb\Table\Row\Exception If $parentTable is not a table or is not loadable.
    */
   public function findParentRow($parentTable, $ruleKey = null, libDb\Table\Select $select = null) {
      $db = $this->_getTable()->getAdapter();
      ...
      if (!$parentTable instanceof \libDb\Table\AbstractTable) {
         $type = gettype($parentTable);
         if ($type == 'object') {
            $type = get_class($parentTable);
         }
         throw new \libDb\Table\Row\Exception("Parent table must be a \libDb\Table\AbstractTable, but it is $type");
      }

      // even if we are interacting between a table defined in a class and a
      // table via extension, ensure to persist the definition
      if (($tableDefinition = $this->_table->getDefinition()) !== null && ($parentTable->getDefinition() == null)) {
         $parentTable->setOptions(array(\libDb\Table\AbstractTable::DEFINITION => $tableDefinition));
      }
      ...
      for ($i = 0; $i < count($map[\libDb\Table\AbstractTable::COLUMNS]); ++$i) {
         $dependentColumnName = $db->foldCase($map[\libDb\Table\AbstractTable::COLUMNS][$i]);
         ...
         $parentColumnName = $parentDb->foldCase($map[\libDb\Table\AbstractTable::REF_COLUMNS][$i]);
         $parentColumn = $parentDb->quoteIdentifier($parentColumnName, true);
         $parentInfo = $parentTable->info();

         // determine where part
         $type = $parentInfo[\libDb\Table\AbstractTable::METADATA][$parentColumnName]['DATA_TYPE'];
         $nullable = $parentInfo[\libDb\Table\AbstractTable::METADATA][$parentColumnName]['NULLABLE'];
   ...
   /**
    * @param  string|\libDb\Table\AbstractTable  $matchTable
    * @param  string|\libDb\Table\AbstractTable  $intersectionTable
    * @param  string                         OPTIONAL $callerRefRule
    * @param  string                         OPTIONAL $matchRefRule
    * @param  libDb\Table\Select           OPTIONAL $select
    * @return libDb\Table\Rowset\RowsetAbstract Query result from $matchTable
    * @throws \libDb\Table\Row\Exception If $matchTable or $intersectionTable is not a table class or is not loadable.
    */
   public function findManyToManyRowset($matchTable, $intersectionTable, $callerRefRule = null, $matchRefRule = null, libDb\Table\Select $select = null) {
      $db = $this->_getTable()->getAdapter();
      ...
      if (!$intersectionTable instanceof \libDb\Table\AbstractTable) {
         $type = gettype($intersectionTable);
         if ($type == 'object') {
            $type = get_class($intersectionTable);
         }
         throw new \libDb\Table\Row\Exception("Intersection table must be a \libDb\Table\AbstractTable, but it is $type");
      }

      // even if we are interacting between a table defined in a class and a
      // table via extension, ensure to persist the definition
      if (($tableDefinition = $this->_table->getDefinition()) !== null && ($intersectionTable->getDefinition() == null)) {
         $intersectionTable->setOptions(array(\libDb\Table\AbstractTable::DEFINITION => $tableDefinition));
      }
      ...
      if (!$matchTable instanceof \libDb\Table\AbstractTable) {
         $type = gettype($matchTable);
         if ($type == 'object') {
            $type = get_class($matchTable);
         }
         throw new \libDb\Table\Row\Exception("Match table must be a \libDb\Table\AbstractTable, but it is $type");
      }

      // even if we are interacting between a table defined in a class and a
      // table via extension, ensure to persist the definition
      if (($tableDefinition = $this->_table->getDefinition()) !== null && ($matchTable->getDefinition() == null)) {
         $matchTable->setOptions(array(\libDb\Table\AbstractTable::DEFINITION => $tableDefinition));
      }
      ...
      for ($i = 0; $i < count($matchMap[\libDb\Table\AbstractTable::COLUMNS]); ++$i) {
         $interCol = $interDb->quoteIdentifier('i' . '.' . $matchMap[\libDb\Table\AbstractTable::COLUMNS][$i], true);
         $matchCol = $interDb->quoteIdentifier('m' . '.' . $matchMap[\libDb\Table\AbstractTable::REF_COLUMNS][$i], true);
         $joinCond[] = "$interCol = $matchCol";
      }
      $joinCond = implode(' AND ', $joinCond);

      $select->from(array('i' => $interName), array(), $interSchema)
              ->joinInner(array('m' => $matchName), $joinCond, libDb\Select::SQL_WILDCARD, $matchSchema)
              ->setIntegrityCheck(false);

      $callerMap = $this->_prepareReference($intersectionTable, $this->_getTable(), $callerRefRule);

      for ($i = 0; $i < count($callerMap[\libDb\Table\AbstractTable::COLUMNS]); ++$i) {
         $callerColumnName = $db->foldCase($callerMap[\libDb\Table\AbstractTable::REF_COLUMNS][$i]);
         $value = $this->_data[$callerColumnName];
         $interColumnName = $interDb->foldCase($callerMap[\libDb\Table\AbstractTable::COLUMNS][$i]);
         $interCol = $interDb->quoteIdentifier("i.$interColumnName", true);
         $interInfo = $intersectionTable->info();
         $type = $interInfo[\libDb\Table\AbstractTable::METADATA][$interColumnName]['DATA_TYPE'];
    ...
    *
    * @param string $method
    * @param array $args OPTIONAL libDb\Table\Select query modifier
    * @return \libDb\Table\AbstractTable|libDb\Table\Rowset\RowsetAbstract
    * @throws \libDb\Table\Row\Exception If an invalid method is called.
    */
   public function __call($method, array $args) {
      $matches = array();

      if (count($args) && $args[0] instanceof libDb\Table\Select) {
         $select = $args[0];
      } else {
         $select = null;
      }
      ...
      throw new \libDb\Table\Row\Exception("Unrecognized method '$method()'");
   }
